...
def main(device, args):
    ...
    for epoch in global_progress:
        ...
        if (epoch == 0 or (epoch + 1) % args.train.random_matrix_epoch_interval == 0) and hasattr(model, 'change_random_matrix'):
            model.change_random_matrix(device=device)
        ...
        if args.train.knn_monitor and epoch % args.train.knn_interval == 0: 
            accuracy = knn_monitor(nn.Sequential(model.backbone), memory_loader, test_loader, device, k=min(args.train.knn_k, len(memory_loader.dataset)), hide_progress=args.hide_progress) 
